...
function resolvePaths(projectRoot: string, args: string[]) {
  ...
  const storeFilePath = resolveAbsolutePath(
    projectRoot,
    args[1] ?? path.join("data", "content-store.json"),
  );
...
async function main() {
  ...
  const { snapshotPath, storeFilePath } = resolvePaths(projectRoot, process.argv.slice(2));
  const snapshot = await loadSnapshot(snapshotPath);
  const repositoryConfig: LearningContentRepositoryConfig = {
    storeFilePath,
  };
  const repository = createJsonLearningContentRepository(repositoryConfig);
  const store = repository.replace({
    curriculumSlices: snapshot.curriculumSlices,
    sourceSnapshotPath: snapshotPath,
  });

  console.log(`已导入课程快照到内容仓储 ${storeFilePath}`);
